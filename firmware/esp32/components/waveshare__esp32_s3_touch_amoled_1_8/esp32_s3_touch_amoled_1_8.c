...
static lv_display_t *bsp_display_lcd_init()
{
    ...
    const lvgl_port_display_cfg_t disp_cfg = {
        ...
        .flags = {
            .sw_rotate = true,
            /* The CO5300 QSPI driver cannot DMA directly from an arbitrary
               heap buffer.  With this enabled both LVGL's draw buffer and
               the SW-rotation buffer are allocated DMA-capable, so the panel
               driver never has to allocate a temporary TX buffer for every
               animated frame.  That temporary allocation was the cause of
               the recording-time freeze when internal heap became fragmented. */
            .buff_dma = true,
            .buff_spiram = true,
    ...
    /* This board drives the CO5300 through QSPI, not the ESP32 RGB LCD
       peripheral.  Registering it as RGB makes esp_lvgl_port call
       lv_display_flush_ready() immediately, so LVGL can overwrite its draw
       buffer while the queued QSPI DMA transfer is still reading it.  Under
       sustained audio/BLE load that race appears as stale or sticky pixels.
       The generic IO path installs the panel-IO completion callback and only
       releases the buffer when the transfer has actually finished. */
